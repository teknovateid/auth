...
trait InstallsApiStack
{
    ...
    protected function installApiStack()
    {
        ...
        // seeder
        $files->ensureDirectoryExists(database_path('seeders'));
        $files->copyDirectory(__DIR__.'/../../stubs/api/database/seeders', database_path('seeders'));

        // migration
        $files->ensureDirectoryExists(database_path('migrations'));
        $files->copyDirectory(__DIR__.'/../../stubs/api/database/migrations', database_path('migrations'));

        // factories
        $files->ensureDirectoryExists(database_path('factories'));
        $files->copyDirectory(__DIR__.'/../../stubs/api/database/factories', database_path('factories'));

        // env
        $files->copy(__DIR__.'/../../stubs/api/.env', base_path('.env'));
